...
int main(int argc, char ** argv) {
    ...
    // Prepare token arrays for adapter
    const int n_prompts = (int)prompts.size();
    std::vector<const llama_token*> token_ptrs;
    std::vector<size_t> token_lengths;
    token_ptrs.reserve(n_prompts);
    token_lengths.reserve(n_prompts);
    
    for (const auto & inp : inputs) {
        token_ptrs.push_back(inp.data());
        token_lengths.push_back(inp.size());
    }

    // Compute embeddings using adapter
    const int n_embd = llama_model_n_embd(model);
    struct llama_embedding_result * result = llama_embedding_compute(
        ctx,
        token_ptrs.data(),
        token_lengths.data(),
        n_prompts,
    ...
    if (result == nullptr) {
        LOG_ERR("%s: failed to compute embeddings\n", __func__);
        llama_backend_free();
        return 1;
    }

    // Extract embeddings from result
    const int n_embd_count = result->n_outputs;
    const float * emb = result->embeddings;
    ...
    if (params.embd_out == "json" || params.embd_out == "json+" || params.embd_out == "array") {
        ...
        for (int j = 0;;) { // at least one iteration (one prompt)
            ...
            for (int i = 0;;) { // at least one iteration (n_embd > 0)
                // Use %.17g for sufficient precision to match JSON library output
                // %.17g uses general format with up to 17 significant digits (enough for float precision)
                // and switches to scientific notation only when more compact
                LOG(params.embd_normalize == 0 ? "%1.0f" : "%.17g", emb[j * n_embd + i]);
                i++;
    ...
    // clean up
    llama_embedding_result_free(result);
    llama_backend_free();
